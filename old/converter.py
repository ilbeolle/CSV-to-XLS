...
def print_help():
    ...
    print(C_TITLE + "\n[ 초보자 친화적 사용 가이드 ]")
    print(C_HELP + "----------------------------------------------------------")
    print(C_TEXT + "이 프로그램은 최신 Excel(.xlsx, .xls) 또는 CSV(.csv) 파일을")
    print(C_TEXT + "오래된 Excel 97-2003(.xls) 형식으로 바꿔줍니다.")
    print(C_TEXT + "암호가 '1234'인 파일도 자동으로 풀고, 변환 후 암호를 없앱니다.")
    
    print(C_TITLE + "\n[ 쉬운 방법: 자동 모드 (가장 추천!) ]")
    print(C_TEXT + "  1. 변환할 파일(들)을 마우스로 선택하세요.")
    print(C_TEXT + "  2. 선택한 파일을 이 프로그램(.exe) 아이콘 위로 끌어다 놓으세요.")
    print(C_TEXT + "  3. 자동으로 변환되어 바탕화면에 저장됩니다.")
    print(C_TEXT + "  4. 작업 끝나면 Enter 키를 눌러 창을 닫으세요.")

    print(C_TITLE + "\n[ 고급 방법: 대화형 모드 ]")
    print(C_TEXT + "  1. 프로그램(.exe)을 더블클릭해서 검은 창(CMD)을 열으세요.")
    print(C_TEXT + "  2. 변환할 파일을 이 검은 창으로 끌어다 놓으세요.")
    print(C_TEXT + "  3. Enter 키를 누르면 변환 시작!")
    
    print(C_TITLE + "\n[ 변환 시 알아둘 점 ]")
    print(C_HELP + "  - 저장 위치: 항상 로컬 바탕화면(C:\\Users\\[당신의 이름]\\Desktop)에 저장됩니다.")
    print(C_HELP + "  - 파일 이름: 원본 이름 그대로 (예: orderList.xlsx → orderList.xls).")
    print(C_HELP + f"  - 이미 같은 이름의 .xls 파일이 있으면? 자동으로 지우고 새로 저장. 지울 수 없으면 숫자 붙여 저장 (예: orderList_1.xls).{C_HELP}")
    print(C_HELP + "  - 변환 끝나면 바탕화면 폴더가 자동으로 열립니다.")
    
    print(C_TITLE + "\n[ 유용한 명령어 (검은 창에서 입력) ]")
    print(C_TEXT + f"  {C_CMD}--help{C_TEXT}: 이 가이드 다시 보기.")
    print(C_TEXT + f"  {C_CMD}clear{C_TEXT}: 화면 깨끗이 지우기.")
    print(C_TEXT + f"  {C_CMD}exit 또는 quit{C_TEXT}: 프로그램 끄기.")
    
    print(C_WARN + "\n[ 주의사항 (문제 발생 시 확인!) ]")
    print(C_WARN + "  - Microsoft Excel이 컴퓨터에 설치되어 있어야 합니다.")
    print(C_WARN + "  - 변환 전에, 저장될 파일(예: orderList.xls)이 Excel에서 열려 있지 않은지 확인하세요.")
    print(C_WARN + "  - OneDrive나 클라우드 폴더에 파일이 있으면 잠길 수 있어요. 로컬 바탕화면에 복사해서 사용하세요.")
    print(C_WARN + "  - 문제가 생기면 'converter.log' 파일을 열어 오류를 확인하세요.")
    print(C_HELP + "----------------------------------------------------------\n")
...
def get_desktop_path() -> str:
    ...
    try:
        ...
        raise RuntimeError("바탕화면 경로를 찾을 수 없습니다.")
...
def create_excel_instance():
    ...
    try:
        ...
        raise RuntimeError(f"Excel을 시작할 수 없습니다: {e}")
    ...
    if file_ext == '.csv':
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"파일을 열 수 없습니다 (암호 오류 또는 손상): {e}")
...
def save_as_xls(wb, output_path: str):
    ...
    if os.path.exists(output_path):
        try:
            ...
            print(C_WARN + f"  - 파일 삭제 실패 (사용 중): {e}")
...
def process_file(excel, file_path: str):
    ...
    try:
        ...
        print(C_ERROR + f"  오류: 파일 없음. {e}")
        logging.error(f"파일 없음: {e}")
    except ValueError as e:
        print(C_ERROR + f"  오류: 열기 실패. {e}")
        logging.error(f"열기 실패: {e}")
    except RuntimeError as e:
        print(C_ERROR + f"  오류: 저장 실패. {e}")
        logging.error(f"저장 실패: {e}")
    except Exception as e:
        print(C_ERROR + "\n[ 실패 ]")
